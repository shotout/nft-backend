...
<div class="col-sm-12" id="add-item-container">
  <div class="card user-list">
      <div class="card-header">
        <h5><a href="{{ url('blockchain/list') }}">{{ __('Blockchains') }}</a> >> {{ __('New Blockchain') }}</h5>
    ...
    <div class="card-block">
      ...
      <form id="itemAddForm" class="form-horizontal" action="{{ url('blockchain/save') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row">
              <div class="col-md-8">
                <div class="form-group row ">
                  <label class="col-sm-2 control-label require">{{ __('Blockchains Name') }}</label>
                  <div class="col-sm-8 pl-sm-3-custom">
                    <input type="text" class="form-control" placeholder="{{ __('Blockchain Name')  }}" name="blockchain_name" id="blockchain_name Name" value="{{ old('Blockchain Name') }}">
                ...
                <div class="form-group row">
                  <div class="col-md-8 offset-sm-2" id='note_txt_1'>
                    <span class="badge badge-danger">{{ __('Note')  }}!</span> {{ __(' Allowed File Extensions: Vector,PNG,JPEG,JPG,SVG') }} || {{__('Max File Size : 10 Mb') }}
                  </div>
                </div>                  
              </div>
            </div>
