...
var cannedEditorValue;
ClassicEditor.create(document.querySelector('#txtAreaCanned'))
.then(editor => {
    cannedEditorValue = editor;
...
var cannedEditorValueEdit;
ClassicEditor.create(document.querySelector('#txtAreaCannedEdit'))
.then(editor => {
    cannedEditorValueEdit = editor;
    theEditor = intialData = editor.getData(); // Save for later use.
    editor.model.document.on('change', () => {
        let editorData = intialData = editor.getData();
        editorData = editorData.replace(/&nbsp;/g, '').replace('<p>', '').replace('</p>', '').replace(/^\s+|\s+$/g, "");
...
$("#cannedForm").on('submit', function(event) {
    ...
    if ($("#cannedForm").valid() == true) {
        let title = $('#title').val();
        jsonData = {
            'title': title,
            'editorValue': theEditor,
        }
...
if ($('.main-body .page-wrapper').find('#reply-ticket-container').length || $('.main-body .page-wrapper').find('#cus-panel-ticket-reply-container').length) {
    ...
    $( "#cannedLink_search" ).autocomplete({
        ...
        select: function (event, ui) {
            ...
            if (e.id) {
                let setText;
                if ((theEditor.length) > 13) {
                    setText = theEditor + e.fullValue;
                } else {
                    setText = e.fullValue;
                }
                theEditorObj.setData(setText);
                this.value = "";
                return false;
            }
        },
    ...
    $("#modal-canned").on("show.bs.modal", function () {
        $(this).find('input[name=title]').val('');
        $(this).find('textarea[name=canned_message]').text('');
        $('#cannedForm').validate().resetForm();
        $(this).find('input[name=title]').removeClass('error');
...
jQuery.validator.addMethod("regexLink", function(value, element) {
    var regExp = new RegExp(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);
    return this.optional(element) || regExp.test(value);
}, jsLang('Please enter a valid link.'));
